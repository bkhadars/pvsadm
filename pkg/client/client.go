...
func (c *Client) CreateServiceInstance(instanceName, serviceName, servicePlan, resourceGrp, region string) (string, error) {
	//Find ServiceName to be created
	...
	if resourceGrp == "" {
		...
		grpList, err := c.ResGroupAPI.List(&resourceGroupQuery)
		if err != nil {
			return "", err
		}
		...
		grp, err := c.ResGroupAPI.FindByName(nil, resourceGrp)
		if err != nil {
			return "", err
		}
		resourceGroup = grp[0]
...
func (c *Client) DeleteServiceInstance(instanceID string, recursive bool) {
	err := c.ResourceClientV1.DeleteInstance(instanceID, recursive)
	if err != nil {
		klog.Infof("Failed to delte the instance with id %s because of the error %s", instanceID, err)
	}
}
